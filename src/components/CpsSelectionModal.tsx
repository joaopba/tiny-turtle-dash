...
const CpsSelectionModal: React.FC<CpsSelectionModalProps> = ({ isOpen, onClose, onCpsSelected }) => {
  ...
  const handleSearch = useCallback(async (cpsToSearch: string) => {
    ...
    try {
      ...
      const { data: localData, error: localError } = await supabase.from('local_cps_records').select('*').eq('user_id', user.id).eq('cps_id', parsedCpsId).single();

      if (localData) {
        toast.success("Paciente encontrado nos registros locais!");
        const record: CpsRecord = { CPS: localData.cps_id, PATIENT: localData.patient, PROFESSIONAL: localData.professional, AGREEMENT: localData.agreement, UNIDADENEGOCIO: localData.business_unit, CREATED_AT: localData.created_at };
  ...
  const handleManualSubmit = async () => {
    if (!manualPatientName || !cpsInput || !user?.id) {
      toast.error("Nome do paciente e CPS são obrigatórios.");
      return;
    }
    const newRecord: CpsRecord = {
      CPS: parseInt(cpsInput, 10),
      PATIENT: manualPatientName,
      AGREEMENT: manualAgreement,
      PROFESSIONAL: "N/A (Cadastro Manual)",
      UNIDADENEGOCIO: "N/A (Cadastro Manual)",
      CREATED_AT: new Date().toISOString(),
    };
    await supabase.from('local_cps_records').upsert({ user_id: user.id, cps_id: newRecord.CPS, patient: newRecord.PATIENT, professional: newRecord.PROFESSIONAL, agreement: newRecord.AGREEMENT, business_unit: newRecord.UNIDADENEGOCIO, created_at: newRecord.CREATED_AT }, { onConflict: 'cps_id, user_id' });
    toast.success("Paciente cadastrado manualmente com sucesso!");
    setIsManualAddOpen(false);
    onCpsSelected(newRecord);
  };
  ...
  return (
    <>
      ...
      <Dialog open={isManualAddOpen} onOpenChange={setIsManualAddOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Cadastro Manual de Paciente</DialogTitle>
            <DialogDescription>Insira as informações do paciente para continuar com a bipagem.</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="space-y-2"><Label htmlFor="cps-manual">Número do CPS</Label><Input id="cps-manual" value={cpsInput} disabled /></div>
            <div className="space-y-2"><Label htmlFor="patient-name">Nome do Paciente</Label><Input id="patient-name" value={manualPatientName} onChange={(e) => setManualPatientName(e.target.value)} placeholder="Nome completo do paciente" /></div>
            <div className="space-y-2"><Label htmlFor="agreement">Convênio (Opcional)</Label><Input id="agreement" value={manualAgreement} onChange={(e) => setManualAgreement(e.target.value)} placeholder="Nome do convênio" /></div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsManualAddOpen(false)}>Cancelar</Button>
            <Button onClick={handleManualSubmit}>Salvar e Selecionar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
